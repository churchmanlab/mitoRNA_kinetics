...
# Source: `Bayes_factor_20220315.py/sh`  
...
# This script is used for batch scripts used to run for all genes together with `MitoRNA_Bayes_factor.sh`.   
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Bayes factor calculation for one degradation rate vs two degradation rates'
    ...
    outpath = os.path.join('/n','groups','churchman','ri23','RNAdecay','BayesFactor20220818')
    ...
    log_file = os.path.join(outpath,'LogErr', 'BayesFactor20220818_%s_batch%s_py.log' % (args.organism, str(args.start_id)))
    ...
    for o in organisms:  
        GS[o] = dict()   
        if o == 'm':
            path_gs = os.path.join('/n','groups','churchman','ri23','bseq','GS20210506')##########CHANGE PATHS
    ...
    ##############CHANGE TO MITO_TOTAL AND MITO_RIBO INSTEAD OF NUCLEAR / CYTO / TOTAL
