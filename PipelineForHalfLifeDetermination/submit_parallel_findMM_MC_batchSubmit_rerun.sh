...
temp_files=`/n/groups/churchman/mc348/TimelapseSeq/Scripts/test_submitMM_batchSubmit.sh ${prefix}`
path=`pwd`
...
# 		echo "#SBATCH -o ${path}/${prefix}/slurm-%j.out" >> $OUT
		...
		echo "" >> $OUT
		echo "/n/groups/churchman/mc348/TimelapseSeq/Scripts/findMismatches_complete_4types_variableGenome.R $x $path $fastapath" >> $OUT
