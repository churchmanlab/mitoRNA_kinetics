...
# NOTE: Modified on 4/28/21 by M. Couvillion to to add column for GtoA mutation for use with s6G experiments ALSO to include SAM flags for non-primary reads (in case we want to add those in at some point)

# NOTE: Modified on 2/26/22 by M. Couvillion to to add other mutations for COVID-drug treatment to add A>G and C>T
...
m = nrow(dat)
out <- cbind(dat, matrix(NA, nrow = m, ncol = 17)) # 17 extra columns will hold read nt quality, 11) read nt position, 12) read nt, 13) and genome pos, 14)genome nt, all genome positions covered by the read, #MM, #T>C, #G>A and a binary indication if the read contains a T>C, '' G>A, #C>T, #A>G and a binary indication if the read contains a C>T, '' A>G and total aligned bases, respectively
	...
	if (tmp[2] == "X") {
	   ...
	   for (k in 1:as.numeric(tmp[1])) {
		   ...
		   #  Now, look up the corressponding genomic nucleotide at the mismatch position
                   if (out[i, 7] == 147 | out[i, 7] == 99 | out[i, 7] == 355 | out[i, 7] == 403 | out[i, 7] == 72 | out[i, 7] == 152 | out[i, 7] == 0) { # Strand information is held in the second column of the sam file (163 and 99 for +, 147 and 83 for -)
...
# Bottom strand: 83 = R1-, 163 = R2+, (primary), 339 = R1-, 419 = R2+ (not primary)
    ...
    nGA = 0 # Number of G>A mismatches
    GA = 0 # Flag for if read contains G>A mismatches
    nCT = 0 
    CT = 0 
    nAG = 0 
    AG = 0 
    ...
    # Step through each mismatch and check if it is a T>C or G>A mismatch, etc
        if (nMM > 0) {
            for (l in 1:nMM) {
      		  ...
      		  if (read_let[l] == "A" & (gen_let[l] == "G")) {
                       nGA = nGA + 1
                       GA = 1
                  }
      		  if (read_let[l] == "T" & (gen_let[l] == "C")) {
                       nCT = nCT + 1
                       CT = 1
                  }
      		  if (read_let[l] == "G" & (gen_let[l] == "A")) {
                       nAG = nAG + 1
                       AG = 1
                  }
            }
    ...
    if (nMM > 0) {
    	...
    	out[i, ncol(out)-16] = paste(read_quality, collapse=', ') #11
        out[i, ncol(out)-15] = paste(read_pos, collapse=', ') #12
    	out[i, ncol(out)-14] = paste(read_let, collapse=', ') #13
    	out[i, ncol(out)-13] = paste(gen_pos, collapse=', ') #14
		out[i, ncol(out)-12] = paste(gen_let, collapse=', ') #15
		out[i, ncol(out)-11] = paste(gen_pos_covered, collapse=', ') #16
		out[i, ncol(out)-10] = paste(all_covered_quality_scores, collapse=', ') #17
		out[i, ncol(out)-9] = nMM
		out[i, ncol(out)-8] = nTC
		out[i, ncol(out)-7] = nGA
		out[i, ncol(out)-6] = TC
		out[i, ncol(out)-5] = GA
		out[i, ncol(out)-4] = nCT
		out[i, ncol(out)-3] = nAG
		out[i, ncol(out)-2] = CT
		out[i, ncol(out)-1] = AG
		out[i, ncol(out)] = total_align
      	...
      	# Save NAs
        out[i, ncol(out)-16] = NA
        out[i, ncol(out)-15] = NA
        out[i, ncol(out)-14] = NA
        out[i, ncol(out)-13] = NA
        out[i, ncol(out)-12] = NA
        out[i, ncol(out)-11] = paste(gen_pos_covered, collapse=', ')
        out[i, ncol(out)-10] = paste(all_covered_quality_scores, collapse=', ')
        out[i, ncol(out)-9] = 0
        out[i, ncol(out)-8] = 0
        out[i, ncol(out)-7] = 0
        out[i, ncol(out)-6] = 0
