...
###					2/27/2022 by Mary Couvillion (to keep track of more types of MM, for COVID drug analysis)
...
# 		echo "#SBATCH -o ${path}/${prefix}/slurm-%j.out" >> $OUT
		...
		echo "" >> $OUT
		echo "/n/groups/churchman/mc348/TimelapseSeq/Scripts/findMismatches_complete_4types_variableGenome.R $x $path $fastapath" >> $OUT # findMismatches_complete_withGtoA_variableGenome.R 
