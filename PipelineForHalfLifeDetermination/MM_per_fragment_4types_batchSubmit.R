...
for (x in seq(from=1, to=number_of_reads, by=2)) {
  ...
  if  (table[x,2] == table[(x+1),2] && table[x,8] == table[(x+1),8]) {
    ...
    if (num_MM == 0) {
      ...
      # which of these mismatches are T>C or G>A mismatches?
      num_TC<-0
      num_GA<-0
      num_CT<-0
      num_AG<-0
      ...
      for (c in seq(from=1, to=nrow(mismatch_table))){
        ...
        if (mismatch_table[c,2]=="A" & mismatch_table[c,4]=="G") {
          num_GA<-num_GA+1
        }
        if (mismatch_table[c,2]=="T" & mismatch_table[c,4]=="C") {
          num_CT<-num_CT+1
        }
        if (mismatch_table[c,2]=="G" & mismatch_table[c,4]=="A") {
          num_AG<-num_GA+1
        }
      }
      ...
      # does the fragment contain a G>A mismatch?
      read_GA<-0
      if (num_GA>0) {
      read_GA<-read_GA+1
      }
      # does the fragment contain a C>T mismatch?
      read_CT<-0
      if (num_CT>0) {
      read_CT<-read_CT+1
      }
      # does the fragment contain a A>G mismatch?
      read_AG<-0
      if (num_AG>0) {
      read_AG<-read_AG+1
      }
      ...
      # report new number of G>A mismatches
      new_table[i,20]=num_GA
      # report of the fragment contains a T>C mismatch (binary)
      new_table[i,21]=read_TC
      # report of the fragment contains a G>A mismatch (binary)
      new_table[i,22]=read_GA

      # report new number of C>T mismatches
      new_table[i,23]=num_CT
      # report new number of A>G mismatches
      new_table[i,24]=num_AG
      # report of the fragment contains a C>T mismatch (binary)
      new_table[i,25]=read_CT
      # report of the fragment contains a A>G mismatch (binary)
      new_table[i,26]=read_AG
